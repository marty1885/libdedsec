...
Format-specific surface syntax is deliberately out of the current Unicode
focus. A later module may tokenize a versioned format with raw-byte provenance
and expose a feature stream, but it must apply the same default-alert versus
